...
Room growth prefers compact silhouettes, preserves progression rank, and converts same-room adjacency to broad 224–256-unit hall portals. The bounded shoulders keep four-way coarse-grid junctions explicit instead of allowing four full-edge openings to collapse into a zero-area boundary at one vertex.

Each composed room also receives a deterministic proportion profile on a 384-unit coarse module. Connectors, longitudinal halls, hubs, locked vestibules, shrines, and arenas vary between 320 and 352 map units per cell while preserving exact same-room joins. The 352-unit cap reserves a 32-unit connector bay between two maximum-width neighbors: one 16-unit door slab plus an 8-unit approach on each side. Starts use the same 352-unit staging footprint, and even compact locks retain 320 units. Ordinary main-route and branch composition now targets at least two compatible cells, replacing the former closet-to-hall lottery with broad continuous spaces while preserving isolated lock ownership.
...
- Pre-emission fluid descriptors shape shallow animated liquids as central, trench, paired, or irregular pools and as straight, staggered, or right-angle rivers. Compact rooms receive a long banked local course; when a composed room has a compatible connected run, the river spans two or three coarse cells and can extend roughly 600–1,000 map units. Techbase and Industrial use water/nukage, Hell uses blood/lava, Gothic mixes blood/water with occasional lava, and Corrupted Tech progresses from water/nukage toward blood/lava. Water and blood are harmless. Nukage deals 5 Slime damage every 32 tics; lava deals 5 Fire damage every 16 tics, ignores radiation-suit protection, and enables terrain damage effects. The complete footprint is reserved before thing placement, contains no initial actors or pickups, leaves a wholly dry placement cell, and retains at least 64 units of dry bank/circulation around every boundary.
...
- **Procedural soundtrack**: after the generated map is loaded, an RNG-independent hash of its archived seed selects one real map marker from the active IWAD and copies that map's music definition. The same seed/IWAD pair therefore keeps the same track across reloads without advancing layout RNG, while shareware and Ultimate Doom naturally use only maps present in their own WAD directories.
...
# Prove both-IWAD liquid availability and every pool/river, reveal, cue, perch, and stair family
